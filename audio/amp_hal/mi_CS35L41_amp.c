 ...
 */
 ...
 static const char* CS35L41_CHANNELS[MAX_CS35L41_AMPS] = {
     "TL", "TR", "BL", "BR"
 };
 ...
 static int format_control_name(const char* channel, const char* control,
     ...
     if (!channel || !control || !buffer) return -EINVAL;
 ...
 static bool check_dsp_health(void) {
     ...
     time_diff = (current_time.tv_sec - g_amp_device->last_health_check.tv_sec) * 1000 +
