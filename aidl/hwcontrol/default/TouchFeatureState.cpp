...
void setTouchFeatureState(int feature, int state) {
    ...
    if (feature != TOUCH_FEATURE_TAP2WAKE && feature != TOUCH_FEATURE_STYLUS) {
        LOG(ERROR) << TAG << ": " << "Invalid feature: " << feature;
        return;
    }
    ...
    if (fd.get() == -1) {
        LOG(ERROR) << TAG << ": " << "Failed to open: " << TOUCH_FEATURE_DEVICE;
        return;
    } else {
        // write the ioctl data to the device
        if (ioctl(fd.get(), TOUCH_FEATURE_IOCTL_SETMODE, &buf) == -1) {
            LOG(ERROR) << TAG << ": " << "Failed to write to: " << TOUCH_FEATURE_DEVICE;
            return;
        } else {
            LOG(INFO) << TAG << ": " << "Wrote touch feature state as: " << ((state > 0) ? "true" : "false")
