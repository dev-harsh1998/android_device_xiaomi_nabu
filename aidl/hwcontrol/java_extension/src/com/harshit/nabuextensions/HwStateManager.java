...
public class HwStateManager {
    ...
    private static void getHwControl() throws IOException {
        if (mHwControl == null) {
            ...
            if (mHwControl == null) {
                if (DEBUG)
                    Log.e(TAG, "IHwControl AIDL interface not available!");
            }
        }
    }
    ...
    // Public method to attempt and set the state of a hardware control from other
    // classes.
    ...
    // public method to just initialize the IHwControl AIDL interface after boot
    // reset.
